...
CREATE TABLE world.nation (
    id BIGSERIAL PRIMARY KEY, name VARCHAR(64) UNIQUE NOT NULL, description VARCHAR(50000)
    );

CREATE SEQUENCE language_seq;
CREATE TABLE world.language (
    id BIGSERIAL PRIMARY KEY, name VARCHAR(64) UNIQUE NOT NULL, description VARCHAR(255)
    );
...
CREATE TABLE world.culture_name (
    name VARCHAR(255), nation_id BIGINT REFERENCES world.nation, PRIMARY KEY (name, nation_id)
    );

CREATE TABLE world.religion (
    id BIGSERIAL PRIMARY KEY, name VARCHAR(255) NOT NULL UNIQUE, description TEXT
    );
